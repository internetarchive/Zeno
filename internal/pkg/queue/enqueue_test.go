...
func TestEnqueue(t *testing.T) {
	...
	t.Run("Check enqueue times", func(t *testing.T) {
		q, _ := NewPersistentGroupedQueue(path.Join(tempDir, "time_test_queue"), make(chan *LogMessage, 100))
		defer q.Close()
	...
	t.Run("Check host order", func(t *testing.T) {
		q, _ := NewPersistentGroupedQueue(path.Join(tempDir, "order_test_queue"), make(chan *LogMessage, 100))
		defer q.Close()
