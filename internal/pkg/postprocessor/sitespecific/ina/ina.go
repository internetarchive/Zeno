...
func IsURL(URL *models.URL) bool {
	return strings.Contains(URL.String(), "ina.fr")
}

func IsAPIURL(URL *models.URL) bool {
	return strings.Contains(URL.String(), "apipartner.ina.fr") && !strings.Contains(URL.String(), "playerConfigurations.json")
}
...
func ExtractMedias(URL *models.URL) (assets []*models.URL, err error) {
	...
	for _, rawAsset := range []string{
		data.ResourceURL,
		data.ResourceThumbnail,
		"https://player.ina.fr" + data.EmbedURL, data.URI,
	} {
