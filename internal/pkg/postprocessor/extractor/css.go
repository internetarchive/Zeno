...
//
// Returns: importAtRuleAreaOK, inAtImportRule
func isValidAtImportArea(gt css.GrammarType, tt css.TokenType, data []byte, pAreaState bool, pImportSate bool) (bool, bool) {
	if !pAreaState {
		return false, false
	}
...
func CSS(cssBody string, inline bool) (links []string, atImportLinks []string) {
	...
	// Whether we are in a valid @import area that allows @import rules
	importAtRuleAreaOK := true
	// Whether we are in a valid @import rule
	var inGoodAtImportRule bool
	for {
		gt, tt, data := p.Next()
		importAtRuleAreaOK, inGoodAtImportRule = isValidAtImportArea(gt, tt, data, importAtRuleAreaOK, inGoodAtImportRule)
		if !importAtRuleAreaOK && !inGoodAtImportRule && gt == css.AtRuleGrammar && tt == css.AtKeywordToken && bytes.Equal(data, atRule) {
