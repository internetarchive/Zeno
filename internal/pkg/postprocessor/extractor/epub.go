...
func ensureBasePath(basePath, assetPath string) string {
	...
	if strings.HasPrefix(cleanAssetPath, cleanBasePath+"/") {
		return cleanAssetPath
	}

	basePathName := filepath.Base(cleanBasePath)
	if strings.HasPrefix(cleanAssetPath, basePathName+"/") {
		return cleanBasePath + "/" + strings.TrimPrefix(cleanAssetPath, basePathName+"/")
	}
