...
// EbookOutlinkExtractor extracts outlinks from EPUB (application/epub+zip) packages.
type EbookOutlinkExtractor struct{}

// Support: run only in general mode (same as PDF extractor).
func (EbookOutlinkExtractor) Support(m Mode) bool {
	return m == ModeGeneral
}

// Match: true for common EPUB MIME types.
...
// Extract: open EPUB (zip) from URL body, locate OPF file, parse manifest+spine,
// read each spine item (XHTML/HTML), parse for <a href="..."> and return URLs.
func (EbookOutlinkExtractor) Extract(URL *models.URL) (outlinks []*models.URL, err error) {
	...
	// Collect errors but continue processing remaining spine items (best-effort)
	var errs []string

	// 4) For each itemref in spine, resolve href and process the file
...
// ---------- helper types & functions for OPF parsing and zip access ----------
...
func findOPFPath(zr *zip.Reader) (string, error) {
	...
	// container.xml structure: <container><rootfiles><rootfile full-path="..."/></rootfiles></container>
...
func parseOPF(opfBytes []byte) (*opfPackage, error) {
	...
	// (we'll rely on struct tags without namespace)
	for {
		t, err := decoder.Token()
		if err == io.EOF {
			break
		...
		// no-op; we are only advancing decoder to ensure well-formedness
		_ = t
	}
	// Simple unmarshal (namespaces are typically fine for these minimal structs)
...
func stripXMLNS(b []byte) []byte {
	...
	// (not fully robust but works for common OPF files)
