...
func addHeadlessFlags(getCmd *cobra.Command) {
	...
	getCmd.PersistentFlags().Int("headless-chromium-revision", headless.MagicLatestChromiumRevision, "[headless] Chromium binary revision to auto download and use (if --headless-chromium-bin is not set). By default, the latest Chromium Stable revision (queried from chromiumdash.appspot.com) will be used.")
