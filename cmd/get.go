...
func addWARCFlags(getCmd *cobra.Command) {
	...
	getCmd.PersistentFlags().Int("max-assets", 0, "Maximum number of assets to archive per page. 0 means no limit.")
	getCmd.PersistentFlags().StringSlice("assets-allowed-file-types", []string{}, "Only archive assets with these file extensions (e.g., jpg,png,css). If specified, assets-disallowed-file-types is ignored.")
	getCmd.PersistentFlags().StringSlice("assets-disallowed-file-types", []string{}, "Do not archive assets with these file extensions (e.g., jpg,png,css). Ignored if assets-allowed-file-types is specified.")
	getCmd.PersistentFlags().Duration("assets-archiving-timeout", 0, "Maximum time to spend archiving assets for a single page. 0 means no timeout.")
