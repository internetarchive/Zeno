...
func getCMDsFlags(getCmd *cobra.Command) {
	...
	getCmd.PersistentFlags().Int("max-concurrent-assets-per-worker", 1, "Max number of concurrent assets to fetch PER worker. E.g. if you have 100 workers and this setting at 8, Zeno could do up to 800 concurrent requests at any time.")
	...
	getCmd.PersistentFlags().MarkDeprecated("ca", "use --max-concurrent-assets-per-worker instead")
